...
export function NBAPlayerValuationCard({ player }: NBAPlayerValuationCardProps) {
  ...
  const getValueDifferenceIcon = () => {
    if (valueDiffPercent > 5) return <TrendingUp className="h-4 w-4 text-green-500" />
    if (valueDiffPercent < -5) return <TrendingDown className="h-4 w-4 text-red-500" />
      ...
      <CardContent className="p-4">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="font-medium">{formatCurrency(player.marketValue)}/yr</span>
          </div>

          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">Predicted Value</span>
            <span className="font-medium">{formatCurrency(player.predictedValue)}/yr</span>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className={`flex items-center font-semibold ${getValueDifferenceColor()}`}>
